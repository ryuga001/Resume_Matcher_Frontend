  ...
  return (
    <div className="bg-white rounded-md p-6 border" style={{ borderColor: COLORS.borderColor, boxShadow: "none" }}>
      <div className="flex items-start justify-between mb-4">
        <div className="size-11 rounded-md flex items-center justify-center" style={{ background: COLORS.accentBg }}>
          {icon}
        </div>
        <span className="text-[10px] font-bold uppercase tracking-[0.14em]" style={{ color: COLORS.textMuted }}>{label}</span>
      </div>
      {loading ? (
        <div className="h-10 w-20 rounded animate-pulse" style={{ background: COLORS.skeletonBg }} />
      ) : (
        value
      )}
