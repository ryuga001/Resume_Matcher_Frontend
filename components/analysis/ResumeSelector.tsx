...
export function ResumeSelector({ resumes, selectedId, onSelect, loadingData }: ResumeSelectorProps) {
  return (
    <div className="flex flex-col bg-white border p-8 rounded-md" style={{ borderColor: COLORS.border, boxShadow: "none" }}>
      ...
      <div className="flex-1 flex flex-col gap-3">
        {loadingData ? (
          [1, 2].map(i => <div key={i} className="h-20 rounded-md animate-pulse" style={{ background: COLORS.bgInput }} />)
        ) : resumes.length === 0 ? (
          <EmptyResumes />
        ) : (
          resumes.map(r => (
...
function ResumeCard({ resume, active, onSelect }: ResumeCardProps) {
  return (
    <button
      ...
      className="text-left p-5 rounded-md border-2 cursor-pointer relative transition-all"
      style={{ borderColor: active ? COLORS.primary : COLORS.border, background: active ? COLORS.primaryLight : "#fff" }}
    >
      <div className="flex items-center gap-4">
        <div className="w-10 h-12 rounded flex items-center justify-center shrink-0" style={{ background: active ? COLORS.primaryLight : COLORS.bgInput }}>
